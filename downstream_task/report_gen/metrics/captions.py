...
# def cal_bleu(gts, res):
#     scorer = Bleu(n=4)
#     score, scores = scorer.compute_score(gts, res)
#     _score_log = np.prod(score) ** (1/len(score))
#     _score = np.exp(np.sum(score))
#     return _score, _score_log
#
# def cal_cider(gts, res):
#     scorer = Cider()
#     score, scores = scorer.compute_score(gts, res)
#     return score
...
def compute_metrics(gts, res, **kwargs):
    ...
    if kwargs.get('target_scorer', False):
        ...
        if 'cider' in kwargs['target_scorer']:
