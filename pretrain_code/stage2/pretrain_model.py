...
from UNet2d_condition import CLIPDDPM_2D, CLIPDDPM_2D_CAM
...
class PretrainModelv5(nn.Module):
    def __init__(self, args, **kwargs):
        super(PretrainModelv5, self).__init__()
        # define parameters
        self.args = args
        ...
        # create vision encoder and momentum vision encoder
        self.vision_encoder_q = CLIPDDPM_2D(model_path="/path_to_ddpm_pretrain_w/")
    ...
    def forward(self, im_q, text_q, text_c=None, im_k=None, text_k=None, mode='train', prompt_info=(None, None), **kwargs):
        ...
        slice_embed = self.vision_encoder_q(im_q, encoder_hidden_states=prompt_info[0], attention_mask=prompt_info[1])

        pre_scan_embed = slice_embed[:, :].reshape(batch_size, seq_len, slice_embed.shape[-1])  # (bs, seq, d)
    ...
    def _dequeue_and_enqueue(self, slice_feat, scan_feat):
        ...
        assert self.queue_size_slice % batch_size_slice == 0, print(self.queue_size_slice, batch_size_slice)  # for simplicity
...
class PretrainModelv5_Zeroshot(nn.Module):
    def __init__(self, args, **kwargs):
        super(PretrainModelv5_Zeroshot, self).__init__()
        # define parameters
        self.args = args
    ...
    def encode_image(self,im_q,prompt_info):
        device = im_q.device
        batch_size, seq_len, c, h, w = im_q.shape
        im_q = im_q.reshape(-1, c, h, w)
        slice_embed = self.vision_encoder_q(im_q, encoder_hidden_states=prompt_info)
        slice_feat = F.normalize(self.vision_proj_slice_q(slice_embed[:, :]), dim=-1)  # cls token as global feature
        ...
        scan_attn = torch.ones(scan_embed.size()[:-1], dtype=torch.long).to(device)
        scan_feat = F.normalize(self.vision_proj_scan_q(scan_feat), dim=-1)
        gather_scan_feat = self._gather_with_grad([scan_feat])[0]

        proj_scan_embed = self.vision_to_text(scan_embed)

        return gather_scan_feat, scan_attn,proj_scan_embed


    def encode_text(self, text_q):
        text_output = self.text_encoder_q(text_q, return_dict=True)
        text_embed = text_output.last_hidden_state
        # use mean pooling of the attention tokens as global text feature, (bs, seq_len, d) -> (bs, d)
        text_feat = self.att_mean_pooler(text_embed, text_q['attention_mask'])
        # (bs, d)
        text_feat = F.normalize(self.text_proj_q(text_feat), dim=-1)
        gather_text_feat = self._gather_with_grad([text_feat])[0]
        return gather_text_feat
    ...
    def forward(self, im_q, text_q, text_c=None, im_k=None, text_k=None, mode='train', prompt_info=(None, None), **kwargs):
        ...
        gather_scan_feat, scan_attn, proj_scan_embed = self.encode_image(im_q, prompt_info)
        gather_text_feat = self.encode_text(text_q)
