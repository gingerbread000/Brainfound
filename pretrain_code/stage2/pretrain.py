...
def train_one_epoch(model, data_loader, tokenizer, tokenizer_mod_prompt, optimizer, scheduler, epoch, device, args, scaler=None):
    # train
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                output = model(im_q=images[0], im_k=images[-1], text_q=encode_tokens[0], text_k=encode_tokens[-1],
                               text_c=decode_tokens, prompt_info=(encoder_hidden_states[0], attention_mask[0]))

            loss = args.weight_itc * output['loss_itc'] + args.weight_lm * output['loss_lm']
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        # '''
        train_stats = train_one_epoch(model, train_loader, tokenizer, tokenizer_mod_prompt, optimizer, lr_sched, epoch, device, args, scaler=scaler)
        dist.barrier()
        if link.is_main_process():
            ...
            for k, v in train_stats.items():
                try:
                    writer.add_scalar(f'log/{k}', v, epoch)
                except:
                    pass
        dist.barrier()
        # '''
